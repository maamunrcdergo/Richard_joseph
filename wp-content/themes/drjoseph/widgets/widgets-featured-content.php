...
class Featured_contents_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
                'featured_content_widget', __('Featured Content Widget', ILLUSIVE_THEME), array('description' => __('A Featured Widget', ILLUSIVE_THEME),)
        );
    }
    ...
    public function widget($args, $instance) {
        ...
        $is_video_class = !empty($is_video) ? 'video_widgets' : '';
        echo '<div class="widget-inner ' . $extra_class.' '.$is_video_class . '">';
        if (!empty($title)) {
            echo $args['before_title'] . $title . $args['after_title'];
        }
        if (!empty($image_url)) {
            $img_alt = empty($sub_title) ? $title : $sub_title;
            echo sprintf('<figure class="widget-img %1$s">', $is_video_class);
            if (!empty($is_video)) {
                //preg_match("/^(?:http(?:s)?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:(?:watch)?\?(?:.*&)?v(?:i)?=|(?:embed|v|vi|user)\/))([^\?&\"'>]+)/", $more_link, $matches);
    ...
    public function form($instance) {
        ...
        <p>            
            ...
            <textarea rows="8" class="widefat" id="<?php echo $this->get_field_id('wcontent'); ?>" name="<?php echo $this->get_field_name('wcontent'); ?>"><?php echo esc_attr($wcontent); ?></textarea>
        </p>
