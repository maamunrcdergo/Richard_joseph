...
// setup
if (ACPT_MESSAGES) {
    add_filter('post_updated_messages', 'acpt_utility::set_messages');
}

if (ACPT_STYLES) {
    add_action('admin_init', 'acpt_utility::apply_css');
}

if (is_admin()) {
    add_action('save_post', 'acpt_save::save_post_fields');
    add_action('admin_enqueue_scripts', 'acpt_utility::upload_scripts');
}


if(!function_exists('get_acpt_plugins')){
    function get_acpt_plugins($acptDefault=''){
     $acptDefault = array('seo');
     return $acptDefault; 
    }
...
if (ACPT_LOAD_PLUGINS == true) {
    ...
    $acptPlugins = get_acpt_plugins();
    if (!empty($acptPlugins)) {
