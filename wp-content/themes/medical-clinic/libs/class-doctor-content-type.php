...
class Doctors_Content_Type {
  ...
  private $post_type = 'mclinic_doctor';
  private $slug = 'doctors';
  private $lan = 'mclinic';
  ...
  public function register() {
    $doctors_labels = array(
      'name' => _x('Doctors', 'post type general name', $this->lan),
    ...
    $doctors_arg = array(
      ...
      'menu_icon' => MCLINIC_THEME_URL . '/images/icon-doctor.png'
    );
...
function get_mclinic_doctors($args = array()) {
  $defaults = array(
    'post_type' => 'mclinic_doctor',
  ...
  if (!empty($doctors_query)) {
    foreach ($doctors_query as $key => $doctor) {
      ...
      $sdata->degree = get_field('degree', $doctor->ID);
      $sdata->speciality = get_field('speciality', $doctor->ID);
